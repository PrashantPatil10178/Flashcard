...
function Flashcards({
  ...
  useEffect(() => {
    const fetchCardData = async () => {
      const frontImages = import.meta.glob("../assets/Carbon compounds/*.webp");
      const backImages = import.meta.glob("../assets/Carbon compounds/*a.webp");
      console.log(frontImages);
      const data = await Promise.all(
        Array.from({ length: 78 }, async (_, index) => {
          const frontImageKey = `../assets/Carbon compounds/${index + 1}.webp`;
          const backImageKey = `../assets/Carbon compounds/${index + 1}a.webp`;
